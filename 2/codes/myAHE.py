...
def adaptive_hist(img_arr,window_size):  #function for calculating Adaptive histogram equalization for gray scale images
	...
	(x,y)=img_arr.shape #getting shape of the image array
	for i in range(x):
		for j in range(y):
			...
			y_e=a[1][len(a[1])-1]
			#print(window)
			cf=hist_eq_1(window[x_s:x_e+1,y_s:y_e+1])  #calculating histogram equalization of window 
			if cf != None:
	...
	for k in range(z):
		...
		for i in range(x):
			...
			#print(i,j,window_center_r,window_center_r+min(window_size[0]-window_center_r,x-i),window_center_c,window_center_c+min(window_size[0]-window_center_c,y-j))
				...
				x_e=a[0][len(a[0])-1]
				y_e=a[1][len(a[1])-1]
				cf=hist_eq_1(window[x_s:x_e+1,y_s:y_e+1])
				if cf != None:
					img_res[i,j,k]=cf[img_arr[i,j,k]]*256 # giving pixel value to the new_image
		#print(i)
	return img_res


def hist_eq_1(img_arr):  # this function used to calculate histogram equilization of given window
	img_res=np.copy(img_arr)
	hist_data=[0]*256  # calculating hist data for all intensity values of a given window
	#print(img_arr.shape)
	...
	cf_sum=[0]*256  #calculating cdf 
	for i in range(len(hist_data)):
		cf_sum[i]=sum(hist_data[:i+1])
	if img_arr.shape[0]==0 or img_arr.shape[1]==0:
		return None
	cf_p=[cf_sum[i]/(img_arr.shape[0]*img_arr.shape[1]) for i in range(len(cf_sum))] #calculating cdf probabilites
...
if __name__ == '__main__':
	...
	if(len(img_barba_arr.shape)) ==2:  ## checking for colored images
		img_barba_res=adaptive_hist(img_barba_arr,(51,51))  #getting adaptive histogram equalized image with given window size passed as a parameter
	...
	if(len(img_tem_arr.shape))==2:
		img_tem_res=adaptive_hist(img_tem_arr,(51,51))
	elif (len(img_tem_arr.shape)) == 3:
		img_tem_res=adaptive_hist_col(img_tem_arr,(51,51))
